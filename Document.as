			...
			if(_blob_array.length > 0) {
				for(var i:int=0; i < _blob_array.length; i++) {
					...
					for(var j:int = 0; j < _notearr.length; j++) {
						...
						trace("trace current note"+ _notearr[j][1][1]);
						trace("trace the note array "+ _notearr[j]);
						trace(" this note is "+_notearr[j][0]);
						
						var traceit = _blob_array[i][1].y + _blob_array[i][1].height - buffer;
